...
Use `/creative-writing-modes` to select the requested pass, `/creative-writing-craft` for scene and prose execution through the supplied resolved stack, `/targeted-editing` to locate, scope, and verify changes to existing prose, `/story-memory` for scoped context, and `/llm-writing` to catch unchosen defaults. Language grammar is the floor; profile shapes register; evidenced broad and narrow voice shapes taste; the current brief controls this pass. Preserve intentional ambiguity, silence, repetition, compression, or fragmentation when it creates the intended effect. Do not update continuity records or run project mechanics; return proposed record changes to muse or the orchestrator, which owns both the transaction and checker execution.

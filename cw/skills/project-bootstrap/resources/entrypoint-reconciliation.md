...
| Only regular `CLAUDE.md` exists | Inspect it. Move directly and unambiguously shared guidance into `AGENTS.md`; retain only the import plus a clearly labeled Claude-specific tail. Ask once only when shared versus Claude-specific intent is materially ambiguous. Never discard content. |

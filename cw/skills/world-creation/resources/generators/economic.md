...
Use this generator when a session needs currencies, trade networks, resource economies, or economic pressures that should drive plot and character motivation. It applies the ten principles that govern how real economies form, function, and evolve, producing economies that drive story rather than serve as backdrop.
...
Choose a position on every category; the currency, trade, and resource choices below are the load-bearing ones for story pressure.
...
1. **Resource base definition** — map key resources and their distribution, determine energy sources and limitations, establish the renewable vs. non-renewable balance, create resource extraction technologies, and design resource-based power relationships.
2. **Production system development** — define primary production methods, establish ownership and control patterns, create labor organization structures, design technological production tools, and build the occupational specialization system.
3. **Exchange system creation** — develop the currency or value representation, create market structures and locations, design trade networks and routes, establish price determination mechanisms, and build transaction protocols and customs.
4. **Distribution system design** — create wealth accumulation patterns, establish inequality levels and justifications, design redistribution mechanisms, build access limitation systems, and develop the luxury vs. necessity distinction.
5. **Organization structure building** — create economic decision-making institutions, establish information flow patterns, design regulatory frameworks, develop enforcement mechanisms, and build economic record-keeping systems.
6. **Culture-economy integration** — connect economic roles to social status, establish moral constraints on economics, create economically significant rituals, and design economically based identity markers.
7. **System evolution planning** — create historical economic stages, design crisis points and adaptations, establish technological transition effects, and build the formal-informal sector balance.
...
- **With governance:** regulatory frameworks and taxation structures shape and are shaped by economic power — design together with `resources/generators/governance.md`.
- **With belief systems:** moral constraints on economic behavior come from the setting's religions — see `resources/generators/belief.md`.
- **With consequences:** trace economic ripple effects through society with `resources/generators/systemic.md`.

## Output

Deliver the proposal into the parent world-creation session: chosen values for each parameter category, the economic system type, currency design (naming, backing, properties), trade networks with routes, dependencies, and barriers, the class structure, and the informal/shadow economy elements, plus at least one economic crisis in history and planned future economic pressures for plot. Nothing here is canon until the author confirms it; once confirmed, the session persists it through its normal lore-file transaction. Check `/story-memory` conventions for cross-session artifact handling, and use `/creative-research` when modeling on a real historical economy. Character-level economic pressure is prose territory — route it to `/creative-writing-craft`.

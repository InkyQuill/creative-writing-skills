...
Load `/qi-layer` when writing or editing CLAUDE.md or .context/ files: it
...
|---|---|---|
| `CLAUDE.md` | Intent and mental model for a directory | First thing agents read on entry; what to understand *before* working here |
...
If it's intent/mental-model for a directory → `CLAUDE.md`.
...
in that subsystem's CLAUDE.md or .context/ even when it reads like a
...
Every durable layer — CLAUDE.md, `.context/`, KB, `docs/` — holds the best
...
**Truth is anchored per layer.** CLAUDE.md and `.context/` describe the
...
The KB is a directory tree like any other: it carries its own CLAUDE.md
and `.context/` pair, written per `/qi-layer` — intent in CLAUDE.md,
governance depth (writing conventions, structure, validation) in
`.context/`. Read the KB's own `CLAUDE.md` before writing. Treat this

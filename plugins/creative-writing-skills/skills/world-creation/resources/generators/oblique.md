# Oblique Worldbuilding Generator

Generator output is provisional: proposals become world knowledge only after author confirmation, and world-creation work never edits manuscript prose.

Use this generator when a session needs worldbuilding quotes and epigraphs that enhance chapters through perspective-driven documentation rather than direct commentary: chapter epigraphs, in-world documents, or any content where limited perspective creates meaning through what the documenter cannot see. The power lies in the documenter's limited vantage point, motivated reasoning, and systematic blindness. Generated epigraph and document text is a proposal delivered to the author — this workflow never inserts it into chapter files itself.
...
**First-order distance** — same events, different perspective. Risk: too direct. Use sparingly for ironic effect.

**Second-order distance** — related phenomena, different context. The sweet spot for oblique relevance: shows systemic patterns and requires reader inference.

**Third-order distance** — thematic or systemic echoes only. Risk: too obscure. Use for subtle resonance.

### Connection Types

**Systemic echo** — shows the same forces operating elsewhere: different scale, same dynamics; parallel structures in different domains; same problem, different manifestation.

**Ironic juxtaposition** — creates meaning through contrast: mundane concerns during crisis; historical confidence before catastrophe; bureaucratic language for human tragedy; technical precision describing chaos.

**Thematic rhyme** — different situation, same human pattern: power dynamics across contexts; human costs in different systems; universal behaviors in specific settings.

**Causal chain** — distant causes or effects: butterfly effect implications; unintended consequences; historical roots of current problems; future echoes of present choices.
...
- **The revealing detail** — include small details only this perspective would notice or care about, revealing their priorities and blindness.
- **The telling omission** — what obvious element (obvious to readers) does the documenter completely fail to mention?
- **The accidental truth** — what does the documenter reveal without meaning to, through their framing or focus?
- **The competing authorities** — use contradictory documents about the same phenomena to reveal factional perspectives.
- **The unreliable compiler** — create a fictional editor whose bias shapes the selection and framing of documents.
- **The layered meaning** — craft quotes that work on multiple levels: surface (interesting worldbuilding detail), contextual (comments on chapter events), ironic (reveals documenter's limitations), thematic (illuminates larger concerns).

## Creation Process

1. **Analyze the chapter.** What system or force is at work? What human cost is shown? What assumptions are challenged? What power dynamics are revealed?
2. **Choose the perspective.** Who would document this type of thing? What would they need to believe? What couldn't they afford to see? How would they frame it?
3. **Select the document type.** Match formality to the perspective, consider whose voice adds most value, and balance variety across the work.
4. **Calibrate connection strength.** Aim for "Ah!" not "Obviously." Ensure the quote works standalone, and test for revelation when connected.
5. **Layer additional meaning.** Plant seeds for future revelations, add worldbuilding texture, and create re-read value.

## Worked Example

**Chapter event:** a character discovers their employer has been suppressing union organizing through surveillance tech.
...
- **The perspective test** — is the documenter's position clear? Are their limitations believable? Does their blindness create irony?
- **The relevance test** — does the quote resonate with chapter themes? Is the connection discoverable but not obvious? Does it expand understanding?
- **The standalone test** — is the quote interesting without the chapter? Does it build the world independently? Could it seed its own story?
...
3. **Systems echo** — the same forces create different documents across contexts.
...
- **With belief and culture generators:** documentary voices draw on the setting's institutions and belief systems for their positions and blindnesses — see `resources/generators/belief.md`, `resources/generators/governance.md`, and `resources/generators/economic.md`.
- **With prose craft:** how an epigraph reads on the page, voice authenticity, and chapter-opening effects belong to `$creative-writing-craft`.

## Output

Deliver the proposal into the parent world-creation session: the documentary voice definition (position, need, lens, blindness), the document type, the connection type (systemic echo, ironic juxtaposition, thematic rhyme, causal chain), the document text itself, and the layered meanings. Distinguish what the method can settle (are all four perspective components defined? is a chapter connection identifiable? does the quote stand alone?) from what the author must judge (subtlety calibration, voice authenticity, whether the connection creates the "ah!" moment). Nothing here is canon until the author confirms it — and the epigraph text itself is delivered as a proposal, never written into chapter files by this workflow. Check `$story-memory` conventions for cross-session artifact handling, and use `$creative-research` when modeling a document type on real-world forms.

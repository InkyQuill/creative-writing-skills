...
Use this generator when a session needs to design cities, towns, and settlements: urban environments, city district maps, or any settlement that needs realistic layered development and spatial logic. It applies the ten principles that govern how real cities and towns form, grow, and evolve, producing urban environments that feel lived-in rather than designed-for-plot.

The method has two load-bearing ideas. **Layered development:** urban spaces evolve through accretion rather than comprehensive planning — every settlement is a palimpsest of founding purpose, growth phases, crises, and adaptive reuse. **Spatial logic:** geography, function, and social hierarchy manifest in the physical organization of space, so the map is evidence of history and power rather than decoration.
...
## Design Process: Layered Development

Work the phases in order; each layer must make sense on top of the previous one.

1. **Site selection and analysis** — identify key geographic features, assess resource availability, evaluate defensive potential, analyze transportation connectivity, and consider climatic conditions. The site rationale is the settlement's founding logic.
2. **Foundation establishment** — determine the founding purpose, select the initial morphology, establish the core structures, and define the original boundaries. This is the oldest layer everything else accretes around.
3. **Growth phase development** — map expansion patterns, add district specialization, develop infrastructure networks, and create social stratification zones. Growth follows the spatial growth patterns above, not a master plan.
4. **Historical layering** — incorporate crisis events, show adaptive responses, create the architectural palimpsest, and develop contested spaces. Each crisis from the adaptation cycles leaves a scar the current city grew around.
5. **Contemporary state** — define the current function, map active and declining areas, identify tensions and conflicts, and show ongoing changes. The present city should read as evidence of everything beneath it.
...
- **With economics:** settlement design needs economic logic, and economies need physical expression in markets and districts — see `resources/generators/economic.md`.
- **With governance:** political power expresses itself through urban form — see `resources/generators/governance.md`.
- **With belief systems:** religious architecture and sacred geography shape the ceremonial layer of the city — see `resources/generators/belief.md`.

## Output

Deliver the proposal into the parent world-creation session: the site analysis and selection rationale, the primary function and morphology, the district organization, the infrastructure systems (water, transport, waste, food), the historical layers with crisis responses, the social geography of elite, common, and marginalized zones, and the cultural expression elements, plus current tensions and ongoing changes. Nothing here is canon until the author confirms it; once confirmed, the session persists it through its normal lore-file transaction. Check `$story-memory` conventions for cross-session artifact handling, and use `$creative-research` when modeling on a real city. Scene staging inside the designed spaces is prose territory — route it to `$creative-writing-craft`.

...
a working `AGENTS.md` and one coherent directory layout that future sessions
...
layouts. Read `AGENTS.md` first when it exists. Look for `_index.md`,
...
write style files, or capture voice goals in `AGENTS.md` yet.
...
project is starting fresh, include voice goals in the proposed `AGENTS.md`
content so style files can be derived from early drafts after approval.

## Propose and Iterate

After the layout is selected, draft project instructions for `AGENTS.md` and
...
updates. Write or update `AGENTS.md` with the agreed instructions. Create only
...
that established name and record its role in `AGENTS.md`.
